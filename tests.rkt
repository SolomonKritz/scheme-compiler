...
; Testing apparatus for compiler
; By Solomon Kritz and Michael Gilray
...
(define ((make-test path) exp ext)
  (lambda ()
    (define t0 (test-compiler top-level desugar assignment-convert alphatize
                              anf-convert cps-convert closure-convert proc->llvm exp))
    t0))

(define ((compile path) exp ext)
  (lambda () (compile-to-bin top-level desugar assignment-convert alphatize anf-convert
                             cps-convert closure-convert proc->llvm exp)))
...
(define tests
  `(,@(map (path->test 'required) (tests-list "required"))
    ,@(map (path->test 'garbage-collection) (tests-list "garbage-collection"))
    ,@(map (path->test 'exceptions) (tests-list "exceptions"))
    ,@(map (path->test 'strings) (tests-list "strings"))))

(define to-compile
  `(,@(map (path->comp 'compile) (tests-list "compile"))))
...
(define (run-test/internal is-repl . args)
  ...
  (match args
    ...
    [(list compile comp-name)
     #:when (assoc comp-name to-compile)
     (match (assoc comp-name to-compile)
       [(list _ _ exec)
        (with-handlers ([exn:fail? (lambda (x) (begin (display "Compilation failed") (newline) (display x) (newline) (exit 1)))])
          (exec))
        (display "Compilation succesful")
        (newline)
        (exit 0)])]
