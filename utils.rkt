...
(require racket/exn)
; Largely by Michael Gilray, with several modifications by Solomon Kritz
...
(define gc-include-path
  (path->string
   (build-path "/" "usr" "local" "include")))

(define clang++-path
  (let ([clang++-path-local "/usr/bin/clang++"])
    (if (file-exists? clang++-path-local)
        clang++-path-local
        "clang++")))

(define prims-list '(= > < <= >= + - * / 
                       cons? null? cons car cdr list first second third fourth
                       length drop memv map append foldl foldr
                       vector? vector make-vector vector-ref vector-set! vector-length
                       list? void? promise? procedure? number? integer?
                       void print halt string-ref string->list
                       eq? eqv? not substring string-append string 
                       haltwithtoomanyerr haltwithtoofewerr))
...
(define (compile-to-bin top-level desugar assignment-convert alphatize anf-convert
                        cps-convert closure-convert proc->llvm top-level-e)  
  (define scm-e (top-level top-level-e))
  (define ir-e (simplify-ir (desugar scm-e)))   
  (define alphatize-and-boxed-e (assignment-convert ir-e))
  (define anf-e (anf-convert alphatize-and-boxed-e))
  (define cps-e (cps-convert anf-e))
  (define proc (closure-convert cps-e))
  (define llvm (proc->llvm proc))
  (compile-llvm llvm))

(define (test-compiler top-level desugar assignment-convert alphatize anf-convert
                       cps-convert closure-convert proc->llvm top-level-e)
  (define val0 (eval-top-level top-level-e))
  (define scm-e (top-level top-level-e))
  (define ir-e (simplify-ir (desugar scm-e)))   
  (define alphatize-and-boxed-e (assignment-convert ir-e))
  (define anf-e (anf-convert alphatize-and-boxed-e))
  ...
  (define proc (closure-convert cps-e))
  (define llvm (proc->llvm proc))
  (define val1 (eval-llvm llvm))
  (if (equal? val0 val1)
      #t
      (begin
        (display (format "Test-Compiler: two different values before and after compilation\n"
                         val0 val1))
        #f)))
...
(define (simplify-ir ir-e)
  (define (T e)
    (match e
      ...
      [`(prim string-append ,es ...)
       (T `((lambda el (apply-prim string-append el)) ,@es))]
      [`(prim string ,es ...)
       (T `((lambda el (apply-prim string el)) ,@es))]
      [`(prim substring ,e0 ,e1 ,e2)
       (T `(prim substring3 ,e0 ,e1 ,e2))]
      [`(prim substring ,e0 ,e1)
       (T `(prim substring2 ,e0 ,e1))]
      [`(prim foldl ,e0 ,e1 ,e2)
      ...
      [`(prim ,op ,es ...)
       `(prim ,op ,@(map T es))]
      [`(apply-prim substring ,lst)
       (T `(if (prim = '2 (prim length ,lst))
               (apply-prim substring2 ,lst)
               (apply-prim substring3 ,lst)))]
      [`(apply-prim ,op ,e0)
       `(apply-prim ,op ,(T e0))]
      [`(,ef ,es ...)
       `(,(T ef) ,@(map T es))]))
...
(define (eval-llvm llvm-str)
  ...
  (when (not recent-header)
    (set! recent-header #t)
    (system (string-append clang++-path " header.cpp " " --std=c++11 " " -I " gc-include-path " -S -emit-llvm -o header.ll"))
...
(define (racket-compile-eval e)
  (define (rewrite-match e)
    ...
    (match e [`(match ,e0 ,clauses ... (,pat0 ,es ...))
      ...
      [else e]))
  (with-handlers ([exn:fail? (lambda (x) (let ([msg (exn-message x)])
                                           (if (regexp-match? #rx"vector-ref: index is out of range" msg)
                                               "RUNTIME ERROR: Attempt to use a vector index that is out of bounds"
                                               (if (regexp-match? #rx"division by zero" msg)
                                                   "RUNTIME ERROR: Cannot divide by zero"
                                                   (if (or (regexp-match? #rx"application: not a procedure" msg)
                                                           (regexp-match? #rx"expected: procedure" msg))
                                                       "RUNTIME ERROR: Cannot apply non-function value"
                                                       (if (regexp-match? #rx"arity mismatch" msg)
                                                           (let* ([lst (regexp-match #px"expected: (\\d)\\s+given: (\\d)" msg)]
                                                                  [expected (string->number (list-ref lst 1))]
                                                                  [given (string->number (list-ref lst 2))])
                                                             (if (> given expected) "RUNTIME ERROR: Too many arguments provided to a function"
                                                                 "RUNTIME ERROR: Too few arguments provided to a function"))
                                                           (begin (pretty-print "Evaluation failed:") (pretty-print x) (pretty-print e) (error 'eval-fail))))))))])
